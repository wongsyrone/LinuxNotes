...
// ArchLinux 必选（可能不全）
# pacman -S base-devel
# pacman -S --needed subversion asciidoc bash bc binutils bzip2 fastjar flex git gcc util-linux gawk gtk2 intltool zlib make cdrkit ncurses openssl patch perl-extutils-makemaker python2 rsync ruby sdcc unzip wget gettext libxslt zlib boost libusb bin86 sharutils jdk7-openjdk b43-fwcutter findutils
// 根据wiki，ArchLinux部分包在AUR里面
$ yaourt -S jikes bcc
```
