...
**pacstrap, pacman执行过程中死了也不要终止它执行，非常重要要说两遍的，懒得复制了**

chroot之后别忘再次设置网络，不然哭死。

单系统没有时钟问题，双系统的话最好bios使用UTC时间，底下单独列了一项。

创建ramdisk注意修改`/etc/mkinitcpio.conf`中的hook。

安装grub的时候一定安装到磁盘而不是分区，也就是不要用sdaX。
...
Arch已经全面systemd了，以前的SysInitV(?)脚本也可以用，但还是学习一下怎么写xxx.service文件吧。

至于GUI，驱动方面尽量没有特殊需求不用“官方驱动”，开源才是王道，顺便Fuck you nVidia。

安装Intel显示驱动之前安Xorg，双显卡笔记本最恶心，暂时用的[bumblebee](https://wiki.archlinux.org/index.php/Bumblebee)。
...
字体需要安装一堆，少安点也可以，它们也不怎么占地方，中文的话喜欢思源黑体，文泉驿大家都用，ttf-fantasque-sans的等宽字体貌似也可以，还没试过。
...
在windows x64中：
"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\TimeZoneInformation\RealTimeIsUniversal"建立一个**QWORD值**，十六进制1，设置cst-8时区，可以开启时间同步
