...
其中 `--tcp-only` 因为性能问题不建议使用，除非必须要用来突破DNS污染  
    ...
    script
            exec /usr/local/sbin/dnscrypt-proxy --resolver-name=cloudns-can --resolver-port=5353 --local-address=127.0.0.9:35535 --user=dnscrypt --logfile=/var/log/dnscrypt.log --pidfile=/var/run/dnscrypt.pid
    end script
...
创建默认位置配置文件 `# touch /etc/dnsmasq.conf`
    ...
    # server=8.8.8.8          # due to serious DNS poisoning, we will not use this
    # server=4.2.2.1
    # server=46.244.10.5       # DNS server from this line may not fit for chinese users
    # server=79.172.201.120
    # server=46.244.10.70
    # server=80.252.121.2
    # server=209.97.202.107
    # server=46.244.10.98
    # server=199.5.157.131
    # server=208.71.35.137
    # server=122.183.133.220
    # server=46.244.10.116
    # server=82.146.40.113
    # server=128.65.160.37
    # server=200.37.61.62
