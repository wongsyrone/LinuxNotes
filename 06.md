...
这个部分的文件是单独的，与其他的文件比如各种 Patch 没有较大的关联，在重新编译的时候只需要`make clean`就行。
...
该文件定义初始化网络信息，主要是交换机和 vlan，不过我遇到怪异的事情是，`ucidef_set_interfaces_lan_wan` 紧跟着的第一个参数是定义 wan 的，第二个是定义 lan 的，比如下面的 eth0 是 wan 而 eth1 是 lan 。
比如 wr2041n v1：
```
tl-wr2041n-v1)
